...
t_data = zeros(length(data.dataReg.codice_regione)/size(regioni_tot,1)-6,size(regioni_tot,1));
...
for reg=1:size(regioni_tot,1)
    ...
    t_data_tot = datenum(data.dataReg.data(index));
...
for i = 1:size(x_data,2)-1
    ...
    [~, ~, ~, t_data_int(:,1)] = splinerMat(t_data_tot(6):t_data_tot(end),t_data_tot(6:end),4,0,t_data_tot(6):0.2:t_data_tot(end));
end
...
for n = 1:size(x_data,1)
    ...
    [0.786310097908281 0.929639769593845 0.119992761116856 0.0348258706467662],...
    'String',datestr(t_data_int(n),'dd mmm'),...
