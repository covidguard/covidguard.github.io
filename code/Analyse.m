mkdir([WORKroot,'/assets/img/regioni']);
mkdir([WORKroot,'/assets/img/province']);
% mkdir('assets');
% mkdir('assets/img');
% mkdir('assets/img/province');
% mkdir('assets/img/regioni');


%% download json from server
%% -------------------------
delete(sprintf('%s/JSON/*.json',WORKroot))

WORKroot = sprintf('C:/Temp/COVID');
...
movefile('dpc-covid19-ita-province.json',sprintf('%s/JSON',WORKroot),'f');
movefile('dpc-covid19-ita-regioni.json',sprintf('%s/JSON',WORKroot)),'f';
...
filename = sprintf('%s/JSON/dpc-covid19-ita-regioni.json',WORKroot);
...
% %     cd([WORKroot,'/assets/img/regioni']);
    print(gcf, '-dpng', [WORKroot,'/assets/img/regioni/reg_',regione, '_cumulati.PNG']);
    close(gcf);
%     cd([WORKroot,'/code']);
    ...
    annotation(gcf,'textbox',[0.72342 0.01426 0.2381 0.04638],...
        ...
        'LineStyle','none',...
        'Color',[0 0 0]);
    print(gcf, '-dpng', [WORKroot,'/assets/img/regioni/reg_',regione, '_giornalieri.PNG']);
    close(gcf);
    catch
    end
end
...
%%%%%%%%%%%%%%%%%%
%% Province
filename = sprintf('%s/JSON/dpc-covid19-ita-province.json',WORKroot);
fid       = fopen(filename, 'rt');
file_scan = textscan(fid, '%s', 'delimiter', '\n', 'endOfLine', '\r\n', 'whitespace', '');
fclose(fid);
file_scan                             = file_scan{1};
...
%     RegioneTot={'Como','Bergamo','Brescia','Lecco'}'
    ...
    print(gcf, '-dpng', [WORKroot,'/assets/img/province/Province_',char(Regione_lista(reg)) ,'_casiTotaliCumulati.PNG']);
    close(gcf);
...
%             test, 'HorizontalAlignment','left','FontSize',7','Color',[.3 .3 .3])
    ...
    annotation(gcf,'textbox',[0.72342 0.01426 0.2381 0.04638],...
        ...
        'LineStyle','none',...
        'Color',[0 0 0]);
    print(gcf, '-dpng', [WORKroot,'/assets/img/province/Province_',char(Regione_lista(reg)) ,'_casiTotaliGiornalieri.PNG']);
    close(gcf);
    catch
    end
end
