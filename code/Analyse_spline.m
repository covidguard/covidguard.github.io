...
hold on
id_f = gcf;
set(id_f, 'Name', sprintf('Italia: Deceduti vs Casi totali'));
title( sprintf('Italia: Deceduti vs Casi totali'));
...
for reg=1:size(regioni_tot,1)
    regione = char(regioni_tot(reg,1));
    index = find(strcmp(dataReg.denominazione_regione,cellstr(regione)));
    x_data(reg,1)=(dataReg.totale_casi(index(end)));% ./pop.popolazioneRegioniPop(reg)*100000;
    y_data(reg,1)=(dataReg.deceduti(index(end)));% ./pop.popolazioneRegioniPop(reg)*100000;
...
set(gca,'Yscale','log');
% 
ylim([min(y_data)*0.5 ceil(max(y_data))*1.5])
xlim([min(x_data)*0.5 max(x_data)*1.5])

x = logspace(0,log10(max(x_data)*1.5),500);
...
print(gcf, '-dpng', [WORKroot,'/slides/img/regioni/ITA_',regione, '_2_ricoverati_con_sintomi.PNG']);
close(gcf);
...
print(gcf, '-dpng', [WORKroot,'/slides/img/regioni/ITA_',regione, '_3_terintVSricoverati_con_sintomi.PNG']);
close(gcf);
...
%     c=plot(t,a2,'-g','LineWidth', 2.0,'color',[0.800000011920929 0.800000011920929 0]);
    ...
    set(gca,'XTick',[time_num(1):interv:time_num(end)+30]);
...
%     l=legend([a,b,c,d],'Dati Reali',sprintf('Stima al %s',datestr(time_num(end),'dd mmm')),sprintf('Stima al %s',datestr(time_num(end-1),'dd mmm')),sprintf('Stima al %s',datestr(time_num(end-2),'dd mmm')));
    l=legend([a,b],'Dati Reali', 'Interpolazione a spline');
...
for k = 1: size(day_unique,1)
    ...
    data_ricoverati(k)=sum(dataReg.ricoverati_con_sintomi(index));
end
time_num = fix(datenum(day_unique));
regione = 'Italia';
...
% 
% 
% %% rapporto deceduti/nuovi casi
% figure;
% id_f = gcf;
% set(id_f, 'Name', 'Italia: rapporto deceduti/nuovi casi');
% 
% set(gcf,'NumberTitle','Off');
% set(gcf,'Position',[26 79 967 603]);
% grid on
...
% y1 = diff(data_casiPositivi);
% y2 = diff(data_deceduti);
% y3 = diff(data_terapie);
% y4 = diff(data_ricoverati);
% 
% 
% a = plot(t,  y2./y1*100,'k');
% b = plot(t,  y3./y1*100,'r');
% c = plot(t,  y4./y1*100,'b');
% 
% [ax,a,b] = plotxx(t,y1,t,y2);
% hold on
% 
...
for reg=1:size(regioni_tot,1)
    ...
    try
        ...
        for type=1:3
            ...
            [a1] = splinerMat(time_num,data,20);
...
for reg = 1:size(Provincia_lista,1)
    ...
    if ~strcmp(cellstr(Provincia_lista(reg,:)),cellstr('')) & ~strcmp(cellstr(Provincia_lista(reg,:)),cellstr('In fase di definizione/aggiornamento')) & ~strcmp(cellstr(Provincia_lista(reg,:)),cellstr('In fase di definizione')) & ~strcmp(cellstr(Provincia_lista(reg,:)),cellstr('In fase di definizione\/aggiornamento')) & ~strcmp(cellstr(Provincia_lista(reg,:)),cellstr('fuori Regione/P.A.'))
...
for reg = 1:size(Regione_lista,1)
    ...
    x_data_tot=[x_data_tot;x_data];
    y_data_tot=[y_data_tot;y_data];
end
...
dist=sqrt(x_data_tot.^2+y_data_tot.^2);
[dist, idxSort]=sort(dist,'descend');
denom_prov_tot(idxSort);



x = x_data_tot(idxSort(1:n_prov),1);
y = y_data_tot(idxSort(1:n_prov),1);
denom_prov_sort = denom_prov_tot(idxSort(1:n_prov));
clear lbl;
...
fontsize=8;
for q=1:size(x,1)
    lbl(q) = text(x(q),y(q), upper(denom_prov_sort{q}(1:4)),'Color', colors{l},'fontsize',fontsize,'FontWeight','bold','horizontalAlignment','center');
    %     end
end
l=l+1;
if l==size(colors,2)
